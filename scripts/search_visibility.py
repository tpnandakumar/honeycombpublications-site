...
MAJOR_HTML = [ROOT / "index.html", ROOT / "books" / "index.html", ROOT / "author" / "index.html", ROOT / "screen-rights" / "index.html"]
...
def audit():
    ...
    for path in MAJOR_HTML:
        ...
        if 'name="description"' not in lower and "name='description'" not in lower:
            warn(f"meta description recommended: {path.relative_to(ROOT)}")
        if 'rel="canonical"' not in lower and "rel='canonical'" not in lower:
            warn(f"canonical recommended: {path.relative_to(ROOT)}")
    required = {f"{BASE}/", f"{BASE}/books/", f"{BASE}/author/", f"{BASE}/screen-rights/"}
    ...
    if errors:
        print(f"Visibility audit failed with {errors} blocker(s).")
        return 1
    print(f"Visibility audit passed: no indexing blockers across {len(MAJOR_HTML)} major pages; {len(urls)} sitemap URLs present.")
    return 0
...
def changed_urls(before, after):
    ...
    if not before or set(before) == {"0"}:
        return all_urls()
    result = subprocess.run(["git", "diff", "--name-only", before, after], cwd=ROOT, text=True, capture_output=True, check=True)
    urls = set()
    shared_change = False
    for name in result.stdout.splitlines():
        ...
        elif name.startswith("assets/") or name in {"robots.txt", "sitemap.xml"}: shared_change = True
    if shared_change: urls |= set(all_urls())
    return sorted(urls)
...
def main():
    ...
    if mode == "submit": return submit(changed_urls(os.environ.get("BEFORE_SHA", ""), os.environ.get("AFTER_SHA", "HEAD")))
